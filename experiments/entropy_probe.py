"""
엔트로피 실측 하네스. `docs/EXPERIMENTS.md` §6.1 "Entropy Validation —
first real-LLM results"(gpt-4o-mini, N=20, 2026-09-17/21)가 이 하네스로
실제로 얻은 결과다 — `dualflow-entropy-probe`(설치 시) 또는
`python experiments/entropy_probe.py`로 재현한다.
...
파이프라인(docs 대화에서 확정한 6단계 formal spec을 그대로 코드화):
    ...
    ④ objective ground truth와 대조 — "이 표현에 몇 개의 후보가 원칙적으로
       ...
       (순환 방지 — 결과를 보기 전에 인벤토리를 고정한다는 걸 커밋 이력으로 보증)
        ▼
    ⑤ 보고 — H와 objective count의 상관관계, 가설(benchmark.py의 "명확/모호" 라벨)과
       실측의 일치/불일치를 숨기지 않고 그대로 기록

이 모듈은 **메인 pilot(experiments/benchmark.py)과 완전히 분리**돼 있다 — Kuhn/
...
from dualflow.semantic import Interpretation, entropy as shannon_entropy, normalize

# --------------------------------------------------------------------------
# 닫힌 vocabulary — experiments/benchmark.py와 동일한 환경을 공유한다. 실측
...
class ProbeResult:
    ...
    hypothesis_label: str  # benchmark.py에서 가져온 "설계자 가설" — 미검증 딱지 유지
...
#: FILE_INVENTORY만 보고 사람이 미리 센 값이다. 모델의 dominant 응답에서
#: 거꾸로 세면 순환논리가 되므로, 반드시 이 표를 조회하는 쪽으로 고정한다
#: (`run_probe`의 `objective_referents` 참고). None은 "이 spec이 대화
#: 맥락(예: '이번에도', '이 요약')을 전제해서, 맥락이 없는 단일턴 probe로는
#: 참조 개수 자체가 정의되지 않는다"는 뜻 — 억지로 숫자를 매기지 않는다.
...
#
# PROBE_CASES의 misread/scope-exceeded/condition 세 spec은 dominant
...
#: trust assumption(§2, benchmark.py)을 여기서 실제로 강제한다 —
...
def main(argv: list[str] | None = None) -> int:
    """`python experiments/entropy_probe.py [--backend openai|anthropic] [--n 20]
    [--model gpt-4o-mini] [--cases main|redesign]` — API 키는 환경변수
    (OPENAI_API_KEY / ANTHROPIC_API_KEY)로만 받는다 — 커맨드라인 인자로도,
    코드에도 절대 남기지 않는다.

    --cases main(기본): PROBE_CASES를 돌려 EXPERIMENTS.md "Entropy
    ...
    p.add_argument("--cases", choices=["main", "redesign"], default="main")
    ...
    results = run_probe_suite(PROBE_CASES, sampler, n=args.n)
