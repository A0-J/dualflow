...
실험 결과를 그림으로 저장한다.
    ...
    python -m dualflow.plots --outdir 어디에 --dpi 200
...
def fig_careless(outdir, dpi, trials=10, warmup=5):
    ...
    for ax, sigma, title in ((axes[0], 0.8, "σ = 0.80  (experience gate active)"),
        ...
        series = {"Slow only": [], "AND": [], "AND + consistency": []}
        for c in cs:
            ...
            for key, cfg in zip(series, cfgs):
                series[key].append(warmup_then_attack(
                    cfg, normal, adv, judge=judge, warmup=warmup,
                    trials=trials)["unsafe_rate"] * 100)
        for (key, ys), color, marker in zip(series.items(),
                                            (PALETTE["slow"], PALETTE["and"],
                                             PALETTE["cons"]), ("o", "s", "^")):
    ...
    fig.suptitle("Experiment ③ — when the reviewer is not perfect", fontsize=12)
    fig.tight_layout()
